...
func (tdb *TestDB) TruncateAll(t *testing.T) {
	tables := []string{"role_permissions", "refresh_tokens", "users", "permissions", "roles", "categories"}
...
func runMigrations(t *testing.T, db *gorm.DB, connStr string) {
	sqlDB, err := db.DB()
	if err != nil {
...
// OpenSQLDB opens a raw sql.DB connection from a GORM instance for goose.
func OpenSQLDB(t *testing.T, db *gorm.DB) *sql.DB {
	sqlDB, err := db.DB()
	if err != nil {
		t.Fatalf("failed to get sql.DB: %s", err)
	}
